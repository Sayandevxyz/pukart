...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border bg-card/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-4 lg:px-8">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            <button aria-label="Notifications" onClick={() => setNotificationsOpen(true)} className="relative rounded-full p-2.5 text-muted-foreground hover:bg-muted hover:text-foreground"><Bell size={18} /><span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-accent" /></button>
            <button aria-label="Messages" onClick={() => setMessagesOpen(true)} className="rounded-full p-2.5 text-muted-foreground hover:bg-muted hover:text-foreground"><MessageCircle size={18} /></button>
            <button onClick={() => setProfileOpen(true)} className="flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-semibold hover:bg-muted"><UserRound size={16} /> Profile</button>
      ...
      {messagesOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-primary/40 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="messages-title" onClick={() => setMessagesOpen(false)}><div className="w-full max-w-lg rounded-t-[2rem] bg-card p-7 shadow-2xl sm:rounded-[2rem] sm:p-9" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-accent">Inbox</p><h2 id="messages-title" className="mt-2 font-serif text-4xl text-primary">Your conversations.</h2></div><button aria-label="Close messages" onClick={() => setMessagesOpen(false)} className="rounded-full p-2 text-primary hover:bg-muted"><X size={20} /></button></div><div className="mt-7 rounded-2xl border border-border p-4"><div className="flex items-center justify-between"><p className="font-semibold text-primary">Mara Studio</p><span className="text-xs text-muted-foreground">New</span></div><p className="mt-2 text-sm leading-6 text-muted-foreground">Thanks for your interest in The quiet art of making. I can answer questions about pickup.</p></div><button onClick={() => showNotice('Messaging is ready for your next conversation')} className="mt-5 w-full rounded-full bg-primary px-5 py-3.5 text-sm font-semibold text-primary-foreground">Start a conversation</button></div></div>}
      {notificationsOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-primary/40 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="notifications-title" onClick={() => setNotificationsOpen(false)}><div className="w-full max-w-lg rounded-t-[2rem] bg-card p-7 shadow-2xl sm:rounded-[2rem] sm:p-9" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-accent">Updates</p><h2 id="notifications-title" className="mt-2 font-serif text-4xl text-primary">You&apos;re all caught up.</h2></div><button aria-label="Close notifications" onClick={() => setNotificationsOpen(false)} className="rounded-full p-2 text-primary hover:bg-muted"><X size={20} /></button></div><p className="mt-6 leading-7 text-muted-foreground">New activity from your saved items and conversations will appear here.</p></div></div>}
      {profileOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-primary/40 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="profile-title" onClick={() => setProfileOpen(false)}><div className="w-full max-w-lg rounded-t-[2rem] bg-card p-7 shadow-2xl sm:rounded-[2rem] sm:p-9" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-accent">Account</p><h2 id="profile-title" className="mt-2 font-serif text-4xl text-primary">Your PUKart profile.</h2></div><button aria-label="Close profile" onClick={() => setProfileOpen(false)} className="rounded-full p-2 text-primary hover:bg-muted"><X size={20} /></button></div><div className="mt-7 flex items-center gap-4 rounded-2xl bg-muted p-4"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary text-primary-foreground"><UserRound size={21} /></div><div><p className="font-semibold text-primary">PUKart member</p><p className="text-sm text-muted-foreground">Build your maker profile and start selling.</p></div></div><button onClick={() => { setProfileOpen(false); setSellOpen(true); setSellSubmitted(false) }} className="mt-5 w-full rounded-full bg-primary px-5 py-3.5 text-sm font-semibold text-primary-foreground">Create a listing</button></div></div>}
