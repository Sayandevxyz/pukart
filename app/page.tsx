    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1440px] items-center gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <a href="#home" className="flex shrink-0 items-center gap-2" aria-label="PUKart home"><img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-EScoY3Dr9cDuwfPiUrfIsTl2QOCJT5.png" alt="PUKart logo" className="h-10 w-10 rounded-xl object-cover" /><span className="sr-only">PUKart</span></a>
...
function Drawer({ type, onClose, onSell, onToast }: { type: 'sell' | 'messages' | 'profile'; onClose: () => void; onSell: () => void; onToast: (message: string) => void }) {
  ...
  async function handleImageChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onToast(error instanceof Error ? error.message : 'Image upload failed')
    } finally {
      setUploading(false)
    }
  } return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[65] flex justify-end bg-primary/40 backdrop-blur-sm" onClick={onClose}><motion.aside initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 28 }} className="h-full w-full max-w-md overflow-y-auto bg-card p-6 shadow-2xl sm:p-8" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-accent">{type === 'sell' ? 'Start selling' : type === 'messages' ? 'Inbox' : 'Your account'}</p><h2 className="mt-2 font-serif text-3xl text-primary">{type === 'sell' ? 'Sell on PUKart' : type === 'messages' ? 'Messages' : 'Hello, PU student'}</h2></div><button onClick={onClose} aria-label="Close panel" className="rounded-full p-2 hover:bg-muted"><X size={20} /></button></div>{type === 'sell' && <form className="mt-8 flex flex-col gap-4" onSubmit={async (event) => {
          event.preventDefault()
          const form = event.currentTarget
          const data = new FormData(form)
          if (!imageUrl) { onToast('Please upload a listing image first'); return }
          try {
            await createListing({
