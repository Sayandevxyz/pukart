  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border bg-card/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-4 lg:px-8">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <button onClick={() => showNotice('Listing flow opened')} className="flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90"><Plus size={16} /> List an item</button>
          </div>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-16 lg:grid-cols-[1fr_0.8fr] lg:px-8 lg:py-24">
          <div className="max-w-2xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <button onClick={() => showNotice('Listing flow opened')} className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/20 bg-card px-6 py-3.5 text-sm font-semibold text-primary hover:bg-muted">Sell something <Plus size={17} /></button>
            </div>
          </div>
      ...
      <section id="discover" className="mx-auto max-w-7xl px-5 py-14 lg:px-8 lg:py-20">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((item) => <article key={item.id} className="group"><div className="relative overflow-hidden rounded-2xl bg-muted"><Image src={item.image} alt={item.title} width={700} height={520} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-105" /><span className="absolute left-3 top-3 rounded-full bg-card/90 px-3 py-1.5 text-xs font-semibold text-primary">{item.tag}</span><button aria-label={`Save ${item.title}`} onClick={() => toggleLike(item.id)} className="absolute right-3 top-3 rounded-full bg-card/90 p-2.5 text-primary shadow-sm hover:bg-card"> <Heart size={17} fill={liked.includes(item.id) ? 'currentColor' : 'none'} /></button></div><div className="flex items-start justify-between gap-3 pt-4"><div><h3 className="font-serif text-xl text-primary">{item.title}</h3><p className="mt-1 text-sm text-muted-foreground">{item.seller} · {item.location}</p></div><p className="text-lg font-semibold text-primary">${item.price}</p></div></article>)}</div>
